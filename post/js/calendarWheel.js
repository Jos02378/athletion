...
class IosSelector {
  constructor(options) {
    let defaults = {
      el: "", // dom
      type: "normal", // infinite 无限滚动，normal 非无限
  ...
  selected(value) {
    for (let i = 0; i < this.source.length; i++) {
      if (this.source[i].value === value) {
        ...
        let initScroll = this._normalizeScroll(this.scroll);
        let finalScroll = i;
        let t = Math.ceil(Math.sqrt(Math.abs((finalScroll + initScroll)) / this.a));
        this._animateToScroll(initScroll, finalScroll, t);
        setTimeout(() => this._selectByScroll(i));
        return;
      }
...
function getMonths(year) {
  ...
  let months = [];
  for (let i = month_num; i <= month_num + 1; i++) {
    months.push({
      value: i + 1,
      text: objectOfAllMonthsAsText[i]
    });
  }
  return months;
}
...
function getDays(year, month) {
  ...
  var date_num = new Date().getDate();
  var Month_num_2 = new Date().getMonth();
  let days = [];

  for (let i = 1; i <= dayCount; i++) {
    if (month === Month_num_2 + 1) {
      if (i >= date_num) {
        days.push({
          value: i,
          text: i
